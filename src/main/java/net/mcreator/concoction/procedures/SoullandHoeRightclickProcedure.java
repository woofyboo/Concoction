...
@EventBusSubscriber(modid = "concoction") // лучше явно указать мод id
public class SoullandHoeRightclickProcedure {
    ...
    public static void onUseHoe(BlockEvent.BlockToolModificationEvent event) {
        ...
        // проверка на почти сломанный Overgrown Hoe в обеих руках
        ...
        if (!event.isSimulated()
            ...
            execute(event, event.getContext().getLevel(),
                    event.getContext().getClickedPos().getX(),
                    event.getContext().getClickedPos().getY(),
                    event.getContext().getClickedPos().getZ(),
                    event.getPlayer());

            if (!event.getPlayer().level().isClientSide()
                    && event.getPlayer() instanceof ServerPlayer serverPlayer
                    && event.getState().getBlock() == Blocks.SOUL_SOIL) {
    ...
    private static boolean isCreative(Entity ent) {
    ...
    private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
        ...
        if (world.getBlockState(BlockPos.containing(x, y, z)).getBlock() == Blocks.SOUL_SOIL
            ...
            world.setBlock(BlockPos.containing(x, y, z),
                    ConcoctionModBlocks.SOULLAND.get().defaultBlockState(), 3);
            ...
            if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() instanceof HoeItem) {
                ...
                if (!isCreative(entity) && world instanceof ServerLevel _level) {
                    (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY)
                            .hurtAndBreak(1, _level, null, _stkprov -> {});
                }
                ...
                // без мотыги, но всё равно анимация и урон предмету в main hand
            } else {
                if (entity instanceof LivingEntity _entity)
                    _entity.swing(InteractionHand.MAIN_HAND, true);
            ...
            if (world instanceof Level _level) {
                if (!_level.isClientSide()) {
                    _level.playSound(null, BlockPos.containing(x, y, z),
                            BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("item.hoe.till")),
                            SoundSource.PLAYERS, 1, (float) Math.random());
                } else {
                    _level.playLocalSound(x, y, z,
                            BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("item.hoe.till")),
                            SoundSource.PLAYERS, 1, (float) Math.random(), false);
                }
            }
        }
    }
