...
public class SunflowerRightClickProcedure {
    ...
    private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
        ...
        // Проверяем, не находится ли игрок в режиме крадущегося движения
        if (entity.isShiftKeyDown()) {
            return; // Если игрок крадется, прекращаем выполнение
        }

        // Проверяем, что это полностью выросший подсолнух
        if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == ConcoctionModBlocks.SUNFLOWER.get()
            ...
            // Уменьшаем возраст нижней части подсолнуха
            ...
            if (world instanceof ServerLevel _level) {
                ItemEntity entityToSpawn = new ItemEntity(_level, (x + 0.5), (y + 0.5), (z + 0.5), new ItemStack(ConcoctionModItems.SUNFLOWER_SEEDS.get()));
                entityToSpawn.setPickUpDelay(10);
                _level.addFreshEntity(entityToSpawn);
            }

            // Дополнительный шанс на выпадение семян
            if (Math.random() < 0.5) {
                if (world instanceof ServerLevel _level) {
                    ItemEntity entityToSpawn = new ItemEntity(_level, (x + 0.5), (y + 0.5), (z + 0.5), new ItemStack(ConcoctionModItems.SUNFLOWER_SEEDS.get()));
                    entityToSpawn.setPickUpDelay(10);
