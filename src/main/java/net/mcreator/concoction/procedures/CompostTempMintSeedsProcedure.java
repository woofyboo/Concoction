...
@EventBusSubscriber
public class CompostTempMintSeedsProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, BlockState blockstate, Entity entity) {
		...
		if (blockstate.getBlock() == Blocks.COMPOSTER) {
			if ((blockstate.getBlock().getStateDefinition().getProperty("level") instanceof IntegerProperty _getip3 ? blockstate.getValue(_getip3) : -1) < 7) {
				if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == ConcoctionModItems.MINT_SEEDS.get()) {
