...
public class CinnamonBoatRenderer extends EntityRenderer<CinnamonBoatEntity> {
	private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(ConcoctionMod.MODID, "textures/entities/cinnamon_boat.png");
	private final BoatModel model;

	public CinnamonBoatRenderer(EntityRendererProvider.Context context) {
		super(context);
		this.shadowRadius = 0.8F;
		this.model = new BoatModel(context.bakeLayer(ModelLayers.createBoatModelName(Boat.Type.CHERRY)));
	}

	@Override
	public void render(CinnamonBoatEntity boat, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		poseStack.pushPose();
	...
	@Override
	public ResourceLocation getTextureLocation(CinnamonBoatEntity boat) {
		return TEXTURE;
	}
}
