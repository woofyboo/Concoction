...
public class BoilingCauldronMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public BoilingCauldronMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		...
		this.customSlots.put(41, this.addSlot(new SlotItemHandler(internal, 5, 118, 38)  {
			...
			public void onTake(Player player, ItemStack stack) {
				...
				if (boundBlockEntity instanceof CookingCauldronEntity cauldron) {
					...
					cauldron.setCraftResult(RecipeOutputData.EMPTY);
				}
				
				if (!player.level().isClientSide() && player instanceof ServerPlayer serverPlayer)
					Utils.grantAdvancement(serverPlayer, "concoction:cauldron_boiling");
			}
		}));
	}
	...
	public boolean stillValid(Player player) {
		if (this.boundBlockEntity instanceof CookingCauldronEntity cauldron
				&& !CookingCauldron.isReadyForCooking(cauldron.getBlockState())) {
			return false;
		}
