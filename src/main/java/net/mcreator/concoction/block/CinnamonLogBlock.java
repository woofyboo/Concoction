...
public class CinnamonLogBlock extends RotatedPillarBlock {
    ...
    // ванильный хук для модификаций блоков инструментами (обтёсывание и т.п.)
    @Override
    public @Nullable BlockState getToolModifiedState(BlockState state,
                                                     UseOnContext context,
                                                     ItemAbility ability,
                                                     boolean simulate) {
        // интересует только топор
        if (!(context.getItemInHand().getItem() instanceof AxeItem)) {
            return super.getToolModifiedState(state, context, ability, simulate);
        }

        // стадия 1: обычное бревно -> кору оставляем только снаружи (bark)
        BlockState bark = ConcoctionModBlocks.CINNAMON_BARK_LOG.get().defaultBlockState()
